...
class GameManager {
  ...
  // 逐关明细（星表）单独存 data/match3/<用户ID>.json，
  ...
  async saveMatch3Record(data) {
    ...
    // 肉鸽试炼可选「得分倍率」「元素颜色降到 3 色」等祝福，分数尺度同样与其它模式不可比，
    // 按三色爽局的隔离思路处理：只进自己的 rogueHighScore / rogueMaxFloor，不并入无尽榜与跨模式累计
    const isRogue = mode === 'rogue';
    const countsToTotals = !isEndless3 && !isRogue;
    ...
    try {
      ...
      // 肉鸽试炼的层数（本轮到达的层数，从 1 起）
      const rogueFloor = Math.max(0, Math.floor(floor || 0));

      // 星级按关逐关取最高，总星数由星表推导（客户端上报的是单关星数）
      // 星表在独立集合里，随关卡数增长的部分不进账号文件
      const starsMap = { ...(await this.accountManager.getMatch3Stars(accountId)) };
      ...
      const maxCombo = isEndless3 || isRogue ? (current.maxCombo || 0) : Math.max(current.maxCombo || 0, combo);
      // 无尽模式最高分独立记录：闯关模式、三色爽局与肉鸽试炼的分数都不参与无尽榜
      const highScore = isLevel || isEndless3 || isRogue
        ? (current.highScore || 0)
      ...
      // 肉鸽试炼自己的最高层数 / 最高分，只有肉鸽结算才更新，其它模式保持原值
      const rogueMaxFloor = isRogue ? Math.max(current.rogueMaxFloor || 0, rogueFloor) : (current.rogueMaxFloor || 0);
      ...
      logger.info('消消乐成绩已保存', { accountId, mode, level, floor: isRogue ? rogueFloor : null, score, stars, maxLevel, totalStars });
